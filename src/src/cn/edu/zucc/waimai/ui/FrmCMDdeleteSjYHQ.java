...
public class FrmCMDdeleteSjYHQ extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					FrmCMDdeleteSjYHQ frame = new FrmCMDdeleteSjYHQ();
	 ...
	 * Create the frame.
	 */
	public FrmCMDdeleteSjYHQ() {
		
	}
	public FrmCMDdeleteSjYHQ(BeanSjYHQ sjyhq) {
		...
		JLabel lblNewLabel = new JLabel("您确定要删除优惠金额是：");
		lblNewLabel.setBounds(29, 31, 216, 33);
		contentPane.add(lblNewLabel);
		
		JLabel lblNewLabel_1 = new JLabel(sjyhq.getYouhui_money()+"元的优惠券吗？");
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(e.getSource()==btnNewButton) {
					try {
						WaiMaiUtil.CMDManager.deleteYHQ(sjyhq);
						setVisible(false);
						JOptionPane.showMessageDialog(null, "删除优惠券成功", "系统提示",JOptionPane.INFORMATION_MESSAGE);
