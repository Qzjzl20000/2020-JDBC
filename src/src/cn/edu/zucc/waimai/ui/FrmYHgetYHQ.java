...
public class FrmYHgetYHQ extends JFrame{
	...
	private Object tblSjFLTitles[]=BeanUserYHQ.UserYHQtableTitles;//商家分栏信息
	...
	private void reloadSjFLTable(int idx){
		...
		curSj=allSj.get(idx);
		try {
			allSjFLs=WaiMaiUtil.userSjManager.loadYHQ(BeanUser.currentLoginUser,curSj);
		} catch (BaseException e) {
			JOptionPane.showMessageDialog(null, e.getMessage(), "错误",JOptionPane.ERROR_MESSAGE);
			return;
		}
	...
	private JMenuItem mntmNewMenuItem_2_1_2 = new JMenuItem("查看持有优惠券（刷新）");
	public FrmYHgetYHQ() {
		...
		mntmNewMenuItem_2_1_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int i=FrmYHgetYHQ.this.dataTableSjJTable.getSelectedRow();
				if(i<0) {
	    ...
	    this.dataTableSjJTable.addMouseListener(new MouseAdapter (){
			...
			public void mouseClicked(MouseEvent e) {//鼠标点击动作，列出右边的列表
				...
				FrmYHgetYHQ.this.reloadSjFLTable(i);
			}
	    });
