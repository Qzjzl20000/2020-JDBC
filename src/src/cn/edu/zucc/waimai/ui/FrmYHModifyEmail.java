...
public class FrmYHModifyEmail extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					FrmYHModifyEmail frame = new FrmYHModifyEmail();
	 ...
	 * Create the frame.
	 */
	public FrmYHModifyEmail() {
		this.setTitle("用户邮箱修改");
		...
		JLabel label = new JLabel("旧邮箱：");
		label.setHorizontalAlignment(SwingConstants.RIGHT);
		label.setBounds(107, 48, 61, 16);
		contentPane.add(label);
		
		JLabel label_1 = new JLabel("新邮箱：");
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(e.getSource()==button) {
					...
					try {
						WaiMaiUtil.userManager.changeEmail(BeanUser.currentLoginUser,opwd,pwd1);
						setVisible(false);
						JOptionPane.showMessageDialog(null, "修改邮件成功", "系统提示",JOptionPane.INFORMATION_MESSAGE);
