...
public class FrmYHModifyCity extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					FrmYHModifyCity frame = new FrmYHModifyCity();
	 ...
	 * Create the frame.
	 */
	public FrmYHModifyCity() {
		this.setTitle("用户城市修改");
		...
		JLabel label = new JLabel("旧城市：");
		label.setHorizontalAlignment(SwingConstants.RIGHT);
		label.setBounds(107, 48, 61, 16);
		contentPane.add(label);
		
		JLabel label_1 = new JLabel("新城市：");
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(e.getSource()==button) {
					...
					try {
						WaiMaiUtil.userManager.changeCity(BeanUser.currentLoginUser,opwd,pwd1);
						setVisible(false);
						JOptionPane.showMessageDialog(null, "修改城市成功", "系统提示",JOptionPane.INFORMATION_MESSAGE);
